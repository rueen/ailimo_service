...
const getReservationList = async (params) => {
  try {
    ...
    const { count, rows } = await db.EquipmentReservation.findAndCountAll({
      ...
      order: [
        [db.sequelize.literal('CASE WHEN `EquipmentReservation`.`status` IN (0, 1) THEN 0 ELSE 1 END'), 'ASC'],
        [db.sequelize.literal('CASE WHEN `EquipmentReservation`.`status` IN (0, 1) THEN JSON_UNQUOTE(JSON_EXTRACT(`EquipmentReservation`.`time_slots`, \'$[0]\')) ELSE NULL END'), 'ASC'],
        ['created_at', 'DESC']
      ],
...
const batchAuditReservations = async (ids, status, rejectReason, handlerId, adminId) => {
  ...
  results.forEach((result, index) => {
    if (result.status === 'rejected') {
      ...
      logger.warn(`Batch audit equipment reservation failed: id=${ids[index]}, reason=${result.reason?.message}`);
    }
  });

  const successCount = ids.length - failedIds.length;
  logger.info(`Batch audit equipment reservations: total=${ids.length}, success=${successCount}`);
