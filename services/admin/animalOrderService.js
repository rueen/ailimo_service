...
const getOrderList = async (params) => {
  try {
    ...
    const { count, rows } = await db.AnimalOrder.findAndCountAll({
      ...
      order: [
        [db.sequelize.literal('CASE WHEN `AnimalOrder`.`status` IN (0, 1) THEN 0 ELSE 1 END'), 'ASC'],
        [db.sequelize.literal('CASE WHEN `AnimalOrder`.`status` IN (0, 1) THEN `AnimalOrder`.`delivery_date` ELSE NULL END'), 'ASC'],
        ['created_at', 'DESC']
      ]
    });
...
const batchAuditOrders = async (ids, status, rejectReason, handlerId, adminId) => {
  ...
  results.forEach((result, index) => {
    if (result.status === 'rejected') {
      ...
      logger.warn(`Batch audit animal order failed: id=${ids[index]}, reason=${result.reason?.message}`);
    }
  });

  const successCount = ids.length - failedIds.length;
  logger.info(`Batch audit animal orders: total=${ids.length}, success=${successCount}`);
