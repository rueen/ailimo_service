...
const getOperationList = async (params) => {
  try {
    ...
    const { count, rows } = await db.ExperimentOperation.findAndCountAll({
      ...
      // 状态优先（待审核=0、进行中=1 排前）；活跃订单按预约最早时间升序（临近优先）；非活跃订单不参与日期排序，回退到创建时间降序
      order: [
        [db.sequelize.literal('CASE WHEN `ExperimentOperation`.`status` IN (0, 1) THEN 0 ELSE 1 END'), 'ASC'],
        [db.sequelize.literal('CASE WHEN `ExperimentOperation`.`status` IN (0, 1) THEN JSON_UNQUOTE(JSON_EXTRACT(`ExperimentOperation`.`time_slots`, \'$[0]\')) ELSE NULL END'), 'ASC'],
        ['created_at', 'DESC']
      ],
 ...
 */
const batchAuditOperations = async (ids, status, rejectReason, handlerId, adminId) => {
  const results = await Promise.allSettled(
    ids.map(id => auditOperation(id, status, rejectReason, handlerId, adminId))
  );
  ...
  results.forEach((result, index) => {
    if (result.status === 'rejected') {
      ...
      logger.warn(`Batch audit experiment operation failed: id=${ids[index]}, reason=${result.reason?.message}`);
    }
  });

  const successCount = ids.length - failedIds.length;
  logger.info(`Batch audit experiment operations: total=${ids.length}, success=${successCount}`);
