...
```json
{
  "configValue": "{\"accessKeyId\":\"new_xxx\",\"accessKeySecret\":\"new_xxx\"}",  // 必填，配置值（JSON字符串）
...
```json
{
  "startDate": "2026-01-01",  // 可选，统计开始日期
  "endDate": "2026-01-31",    // 可选，统计结束日期
  "handlerId": 1              // 可选，负责人ID筛选（不传则统计所有负责人）
}
```
    ...
    "list": [
      {
        ...
        "summary": {
          "totalOrders": 140,      // 总订单数（所有业务）
          "completedOrders": 120,  // 已完成订单数
          "inProgressOrders": 20   // 进行中订单数
        }
      },
      {
        "handler": {
