...
**说明**:
- 请求体为 JSON 对象，可包含任意字段
- 前端可根据需求自由定义字段结构
- 后端直接保存整个 JSON 对象，不做字段校验
- 建议的基础字段：company_name, company_address, contact_phone, email, work_time, company_intro, service_concept, banner_image, video_url
    ...
    "list": [
      {
        ...
        "total": {
          "completed": 120,    // 总已完成订单数（所有业务）
          "in_progress": 20    // 总进行中订单数
        }
      },
      {
        "handler_id": 2,
...
**业务逻辑**:
1. 如果指定 handler_id，只统计该负责人的数据（返回1条记录）
2. 如果不指定 handler_id，分页查询所有负责人的统计数据
3. 统计各业务模块中 handler_id 等于该负责人的订单：
   - equipment（设备预约）
   - cage（笼位预约）
   - experiment（实验代操作）
   - animal（动物订购）
   - reagent（试剂耗材订购）
4. 统计状态：已完成（status=3）、进行中（status=1）
5. 按负责人创建时间倒序排列

**权限要求**: `handler:statistics`
