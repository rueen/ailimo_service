...
```json
{
  "page": 1,                // 页码
  "pageSize": 10,           // 每页数量
  "keyword": "张三",         // 可选，关键字搜索（同时搜索姓名和手机号）
  "name": "张三",           // 可选，姓名模糊查询（keyword 优先）
  "phone": "138",           // 可选，手机号模糊查询（keyword 优先）
  "organization_id": 1,      // 可选，组织机构ID筛选
  "department_id": 1,        // 可选，学院ID筛选
  "research_group_id": 2,     // 可选，课题组ID筛选
  "status": 1,              // 可选，状态筛选：0-禁用 1-启用
  "audit_status": 0          // 可选，审核状态：0-待审核 1-审核通过 2-审核拒绝
}
```

**说明**：
- `keyword` 参数优先级更高，会同时搜索 name 和 phone 字段
- 如果提供了 `keyword`，则 `name` 和 `phone` 参数会被忽略
    ...
    "list": [
      {
        ...
        "audit_status": 1,
        "reject_reason": null,
        "audit_time": "2026-01-05 10:00:00",
        "audit_by": {
          "id": 1,
          "username": "admin"
        },
...
```json
{
  ...
  "city_id": 330100,                 // 可选，城市ID
  "district_id": 330106,             // 可选，区县ID
  "address": "文三路XXX号",         // 可选，详细地址
  "organization_id": 1,              // 必填，组织机构ID
  "department_id": 1,                // 必填，学院ID
  "research_group_id": 1,             // 必填，课题组ID
  "audit_status": 1                  // 可选，审核状态，默认1（审核通过）
}
```
...
3. 验证学院是否属于所选的组织机构
4. 验证课题组是否属于所选的学院
5. 如果提供了省市区ID，验证其有效性和层级关系
6. 如果auditStatus为1，记录审核时间（audit_time）和审核人（audit_by，当前登录的管理员）
7. 创建用户
...
```json
{
  ...
  "district_id": 330106,             // 可选，区县ID
  "address": "文三路XXX号",         // 可选，详细地址
  "organization_id": 2,              // 可选，组织机构ID
  "department_id": 2,                // 可选，学院ID
  "research_group_id": 2,             // 可选，课题组ID
  "status": 1                       // 可选，状态
}
```
...
2. 如果修改组织机构、学院或课题组，验证其存在性和层级关系
3. 如果同时修改组织机构和学院，验证学院是否属于所选的组织机构
4. 如果同时修改学院和课题组，验证课题组是否属于所选的学院
5. 如果修改省市区ID，验证其有效性和层级关系
6. 不能通过此接口修改审核状态
...
### 3.1 学院列表
**接口地址**: `GET /api/support/departments`
...
```json
{
  ...
  "pageSize": 10,         // 每页数量
  "name": "生命",         // 可选，学院名称模糊查询
  "organization_id": 1    // 可选，所属组织机构ID筛选
}
```
...
### 3.2 创建学院
**接口地址**: `POST /api/support/departments`

**请求参数**:
```json
{
  "name": "医学院",          // 必填，学院名称
  "organization_id": 1      // 必填，所属组织机构ID
}
```
...
### 3.3 更新学院
**接口地址**: `PUT /api/support/departments/:id`

**请求参数**:
```json
{
  "name": "临床医学院",      // 可选，学院名称
  "organization_id": 2      // 可选，所属组织机构ID
}
```
...
### 3.4 删除学院
**接口地址**: `DELETE /api/support/departments/:id`
...
**业务逻辑**:
1. 检查是否有用户关联此学院，有则不允许删除
2. 检查是否有课题组关联此学院，有则不允许删除
3. 物理删除
...
### 3.5 学院详情
**接口地址**: `GET /api/support/departments/:id`
...
**权限要求**: `department:detail`

---

### 3.6 根据组织机构获取学院选项列表
**接口地址**: `GET /api/support/departments/options`
...
```json
{
  ...
  "department_id": 1,          // 必填，所属学院ID
  "remark": "专注于神经科学领域研究"  // 可选，备注
}
```
...
```json
{
  ...
  "department_id": 2,            // 可选，所属学院ID
  "remark": "更新后的备注信息"   // 可选，备注
}
```
