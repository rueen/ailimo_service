...
```json
{
  ...
  "user_keyword": "张三",   // 可选，用户关键字搜索（同时搜索姓名和手机号）
  "user_name": "张三",     // 可选，预约用户姓名模糊查询（user_keyword 优先）
  "user_phone": "138",     // 可选，预约用户手机号模糊查询（user_keyword 优先）
...
**说明**：
- `user_keyword` 参数优先级更高，会同时搜索用户的 name 和 phone 字段
- 如果提供了 `user_keyword`，则 `user_name` 和 `user_phone` 参数会被忽略
...
**说明**: 设备租赁提前预约天数配置使用统一的系统配置接口，请参考《接口设计-管理端-内容管理.md》中的"系统配置管理接口"章节。
