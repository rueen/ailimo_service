...
6. **查询匹配的笼位(动物类型+环境+房间)**，如果指定了room_id则必须匹配房间
...
1. 验证动物类型和环境是否存在
2. 验证房间是否存在
3. 检查同一动物类型+环境+房间的笼位是否已存在,存在则提示合并或调整数量
...
**业务逻辑**:
1. 验证动物类型和环境ID是否存在
2. 查询匹配的笼位总数量(animal_type_id + environment_id + room_id)
3. 如果指定了room_id，则必须匹配房间；否则查找该动物类型和环境的所有笼位
4. 如果没有匹配的笼位,返回 total_quantity = 0,available_quantity = 0
5. 遍历日期范围内的每一天(如果end_date为null,只检查start_date当天)
6. 对于每一天,计算已预约数量:
   - 查询该笼位在该日期的所有订单(状态为:0-待审核、1-进行中)
   - 如果传递了exclude_reservation_id,则排除该订单
   - 累加这些订单的 quantity
7. 计算每天的可用数量:total_quantity - 已预约数量
8. 返回所有天中的**最小可用数量**(确保整个日期范围都有足够笼位)
...
### 4.1 笼位房间列表
**接口地址**: `GET /api/support/cage-rooms`
...
### 4.2 创建笼位房间
**接口地址**: `POST /api/support/cage-rooms`
...
**权限要求**: `cage_room:create`

---

### 4.3 更新笼位房间
**接口地址**: `PUT /api/support/cage-rooms/:id`
...
**权限要求**: `cage_room:update`

---

### 4.4 删除笼位房间
**接口地址**: `DELETE /api/support/cage-rooms/:id`
...
**业务逻辑**:
1. 检查该房间下是否有笼位,有则不允许删除

**权限要求**: `cage_room:delete`

---

### 4.5 获取房间选项列表
**接口地址**: `GET /api/support/cage-rooms/options`
