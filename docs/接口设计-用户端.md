...
1. 验证手机号格式
2. **先验证业务规则（不影响频率限制）**：
   - **type=2（注册）时：检查手机号是否已注册，已注册则返回错误**
   - **type=1（登录）时：检查手机号是否已注册，未注册则返回错误**
3. **业务规则通过后，才检查发送频率**：
   - 检查同一手机号1分钟内是否已发送过验证码（防止频繁发送）
   - 检查同一手机号当天发送次数是否超过10次
4. 生成6位随机数字验证码
5. **当前版本：直接返回验证码（模拟短信）**
6. **验证码发送成功后才保存记录**（这样created_at就是真实的发送时间）
7. 未来版本：调用第三方短信服务发送验证码

**频率限制**: 
- 基于手机号（非IP）：同一手机号1分钟内只能发送1次，每天最多10次
- **重要**：只有在业务规则验证通过且验证码发送成功后才计入频率限制
- 这样可以避免因业务规则失败（如手机号已注册）而误触发频率限制
...
1. 验证动物类型、环境、用途是否存在
2. 验证开始日期不能是过去的日期
3. 验证结束日期不能早于开始日期(如果提供了结束日期)
4. **查询匹配的笼位(动物类型+环境+房间)**，如果指定了room_id则必须匹配房间
5. **检查笼位在选定日期范围的剩余数量是否足够(使用数据库行锁防止并发)**
6. 自动分配笼位ID和房间ID
7. 创建订单，状态为"待审核"
8. 自动关联当前登录用户
...
2. 用户选择环境类型 → 调用房间列表接口获取所有房间，用户选择房间
3. 用户选择开始日期、结束日期 → 调用可用数量接口查询剩余数量
4. 用户输入预约数量 → 前端校验数量是否超过可用数量
5. 提交订单 → 后端再次验证可用数量（防止并发问题）
...
```json
{
  ...
  "need_ear_tag": 0,                  // 可选，是否打耳标：0-不需要 1-需要，默认0
  "quantity": 10,                     // 可选，数量，默认1
  "remark": "需要雌性小鼠"           // 可选，备注
}
```
...
```json
{
  ...
  "brand_name": "Thermo Fisher",     // 必填，品牌名称
  "specification_name": "100次/盒",  // 必填，规格名称
  "quantity": 5,                     // 必填，数量
  "orderer_name": "张三",             // 必填，订购人姓名
  "contact_phone": "13800138000",     // 必填，联系电话
  "delivery_date": "2026-01-15",      // 必填，到货日期
...
{
  "code": 504,
  "message": "获取价格表超时，请稍后重试"
}
```

**说明**: 
- 价格表文件 URL 存储在公司信息配置的 `price_list` 字段中
- 服务端会从 OSS 获取文件并原样返回给客户端
- 前端可以直接下载或在新窗口打开
- 建议前端显示下载进度
...
```javascript
// 直接触发下载
window.open('/api/h5/price-list', '_blank');

// 或使用 fetch 显示进度
fetch('/api/h5/price-list')
  .then(response => {
    if (!response.ok) {
...
```json
{
  ...
  "end_date": "2026-02-10"        // 可选，结束日期(YYYY-MM-DD格式)，不传或传null表示长期预约
}
```
...
```json
{
  ...
  "message": "success",
  "data": {
    "total_quantity": 100,        // 该动物类型+环境+房间的总笼位数量
...
**业务逻辑**:
1. 验证动物类型和环境ID是否存在
2. 查询匹配的笼位(animal_type_id + environment_id + room_id)
3. 如果指定了room_id，则必须匹配房间；否则查找该动物类型和环境的所有笼位
4. 如果没有匹配的笼位，返回 total_quantity = 0，available_quantity = 0
5. 遍历日期范围内的每一天，计算已预约数量
6. 返回所有天中的最小可用数量(确保整个日期范围都有足够笼位)

**使用场景**:
- 前端在用户选择动物类型、环境类型、房间(可选)和日期范围后，调用此接口展示可用数量
- 用户根据可用数量填写预约数量
- 提交前前端校验数量是否超过可用数量
