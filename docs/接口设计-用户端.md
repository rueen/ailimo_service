...
```json
{
  ...
  "quantity": 20,                                // 必填，动物数量
  "reservation_date": "2026-01-10",               // 必填，预约日期
  "time_slots": ["09:00-12:00", "13:00-18:00"],  // 必填，预约时间段数组
...
```json
{
  "page": 1,              // 页码
  "page_size": 10,         // 每页数量
  "type": "equipment",    // 可选，订单类型：equipment-设备租赁 cage-笼位预约 experiment-实验代操作 animal-动物订购 reagent-试剂耗材
...
**业务逻辑**:
1. 聚合查询所有类型的订单（设备租赁、笼位预约、实验代操作、动物订购、试剂耗材订购）
2. 只返回当前登录用户的订单
3. 根据订单类型返回对应的标题和日期字段：
   - 设备租赁：title=设备名称，date=预约日期
   - 笼位预约：title=动物类型-环境-数量，date=预约日期
...
```json
{
  ...
  "data": {
    ...
    "quantity": 20,
    "reservation_date": "2026-01-10",
    "time_slots": ["09:00-12:00", "13:00-18:00"],
    "status": 1,
    "status_text": "进行中",
    "remark": "需要进行灌胃操作",
    "handler": {
