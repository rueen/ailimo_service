...
### 2.2 equipment_reservations (设备租赁订单表)
```sql
CREATE TABLE `equipment_reservations` (
  ...
  INDEX `idx_status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='设备租赁订单表';
```

### 2.3 equipment_time_slots (设备租赁时间段表)
```sql
CREATE TABLE `equipment_time_slots` (
  ...
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='设备租赁时间段表';
```
...
CREATE TABLE `cage_reservations` (
  ...
  INDEX `idx_date` (`reservation_date`),
  INDEX `idx_status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='笼位租赁订单表';
```
...
CREATE TABLE `cage_time_slots` (
  ...
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='笼位租赁时间段表';
```
...
CREATE TABLE `company_info` (
  `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY COMMENT '公司信息ID',
  ...
  `service_concept` TEXT COMMENT '服务理念',
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
...
CREATE TABLE `permissions` (
  ...
  `resource` VARCHAR(200) NOT NULL COMMENT '资源路径（路由）',
...
('equipment_advance_days', '7', '设备租赁提前预约天数（单位：天）'),
('cage_advance_days', '7', '笼位租赁提前预约天数（单位：天）'),
