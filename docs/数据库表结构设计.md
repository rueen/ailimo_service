...
CREATE TABLE `users` (
  ...
  `department_id` INT UNSIGNED DEFAULT NULL COMMENT '所属学院ID',
  `research_group_id` INT UNSIGNED DEFAULT NULL COMMENT '所属课题组ID',
  `user_input_organization_name` VARCHAR(100) DEFAULT NULL COMMENT '用户输入的组织机构名称',
  `user_input_department_name` VARCHAR(100) DEFAULT NULL COMMENT '用户输入的学院名称',
  `user_input_research_group_name` VARCHAR(100) DEFAULT NULL COMMENT '用户输入的课题组名称',
...
### 3.1 cage_rooms (笼位房间表)
```sql
CREATE TABLE `cage_rooms` (
...
CREATE TABLE `cages` (
  ...
  `room_id` INT UNSIGNED NULL COMMENT '房间ID',
...
CREATE TABLE `cage_reservations` (
  ...
  `start_date` DATE NOT NULL COMMENT '预约开始日期',
  `end_date` DATE DEFAULT NULL COMMENT '预约结束日期（NULL表示长期预约）',
  `status` TINYINT DEFAULT 0 COMMENT '状态：0-待审核 1-进行中 2-已拒绝 3-已完成 4-已取消',
  `remark` VARCHAR(500) DEFAULT NULL COMMENT '备注',
  `handler_id` INT UNSIGNED DEFAULT NULL COMMENT '负责人ID',
  `reject_reason` VARCHAR(500) DEFAULT NULL COMMENT '拒绝原因',
...
CREATE TABLE `animal_orders` (
  ...
  `need_ear_tag` TINYINT DEFAULT 0 COMMENT '是否打耳标：0-不需要 1-需要',
...
- `users` → `departments` (多对一)
- `users` → `research_groups` (多对一)
- `users` → `regions` (多对一，province_id/city_id/district_id)
- `departments` → `organizations` (多对一)
- `research_groups` → `departments` (多对一)
...
- `cage_reservations` → `cage_rooms` (多对一)
- `cages` → `animal_types` (多对一)
- `cages` → `environment_types` (多对一)
- `cages` → `cage_rooms` (多对一)
